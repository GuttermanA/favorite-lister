...
export default class MovieContainer extends Component {
  ...
  onDragEnd = (result) => {
      ...
      if (result.destination === null) {
        let foundMovie = this.props.movies.find(movie => movie.id === parseInt(result.draggableId.split("-")[1]))
      ...
      this.props.updateFavoriteList(favoriteList)
			...
			let foundMovie = this.props.movies.find(movie => movie.id === result.draggableId)
	...
	render() {
		const movies = this.props.movies.map((movie, index) => {
			return (
				<MovieCard key={uuid()} movie={movie} handleAdd={this.props.addToList} index={index} id={movie.id}/>
			)
		})
		return (
			<DragDropContext
